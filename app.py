...
# ======================== EURO EDGE ENGINE ========================
EURO_TARGETS = [("premier league", "England"), ("la liga", "Spain"), ("serie a", "Italy"),
                ("allsvenskan", "Sweden"), ("eredivisie", "Netherlands"),
                ("premiership", "Scotland")]


def euro_league_hit(league):
    l = (league or "").lower()
    for sub, c in EURO_TARGETS:
        if sub in l:
            return c
    return None
...
def euro_load_db():
    """Pull euro.db from GitHub raw URL for Streamlit's read-only env."""
    url = "https://raw.githubusercontent.com/loansevices74-boop/Owenzo---soccer-ai-app/main/euro.db"
    try:
...
def euro_metrics(db, team, league):
    rows = euro_q(db, "SELECT home, away, fh, fa FROM results WHERE league=? AND (home=? OR away=?) ORDER BY date",
                  (league, team, team))
    g = len(rows)
    if g == 0:
        return None
    ...
    for r in rows:
        if r["home"] == team:
            ...
            gf += r["fa"]
            ga += r["fh"]
    return {"team": team, "league": league, "g": g,
...
def euro_mw_of(db, league):
    rows = euro_q(db, "SELECT home AS t, COUNT() AS g FROM results WHERE league=? GROUP BY home UNION ALL SELECT away, COUNT() FROM results WHERE league=? GROUP BY away",
                  (league, league))
    return (max([r["g"] for r in rows]) if rows else 0) + 1


def euro_matrices(hm, am):
    picks = []
    ...
    if hm["g"] >= 2 and am["g"] >= 2:
        ...
        if abs(hm["diff"] - am["diff"]) <= 0.3 and hm["ga"] < 1.0 and am["ga"] < 1.0 and hm["gf"] <= 1.5 and am["gf"] <= 1.5:
            picks.append(("DRAW", 80 + int(9 * (1 - abs(hm["diff"] - am["diff"]) / 0.3)), combined))
    return picks


def euro_fetch_fixtures():
    out = []
    for i in range(0, 2):
        ...
        try:
            ...
            evs = []
        for e in evs:
            c = euro_league_hit(e.get("strLeague"))
            if c and e.get("strHomeTeam") and e.get("strAwayTeam"):
...
st.caption("v6 Dixon-Coles model • fact-check → team analysis → expected score → mixed slips • Booking odds neglected • Simulated games removed • + Euro Early-Season Edge")
...
tab_pred, tab_track, tab_euro = st.tabs(["⚽ Predict & Slips", "📈 Tracker & ROI Dashboard", "🇪🇺 Euro Edge"])
...
with tab_euro:
    st.header("⚽ EURO EARLY-SEASON EDGE")
    st.caption("Statistical tracking & predictive engine | Premier League • La Liga • Serie A • Allsvenskan • Eredivisie • Scottish Premiership | Entertainment only, 18+")

    euro_db = euro_load_db()

    with st.sidebar:
        st.markdown("---")
        st.header("📡 Euro Season Tracking")
        if euro_db:
            leagues = euro_q(euro_db, "SELECT DISTINCT league FROM results")
            if leagues:
                for r in leagues:
                    st.write("*" + r["league"] + "* — Matchweek " + str(euro_mw_of(euro_db, r["league"])))
                try:
                    last = euro_q(euro_db, "SELECT MAX(date) AS d FROM results")[0]["d"]
                    st.caption("Data up to: " + str(last))
                except Exception:
                    pass
            else:
                st.info("Capturing MW1-2 data...")
        else:
            st.warning("euro.db not found. Run euro-bot workflow first.")

    et1, et2, et3, et4 = st.tabs(["🎯 Matrix Picks", "🔥 Attack Cluster", "🛡️ Fortress Cluster", "📊 All Teams"])

    with et1:
        st.subheader("Matchweek 3+ Confidence Matrices")
        if euro_db:
            found = 0
            for f in euro_fetch_fixtures():
                hm = euro_metrics(euro_db, f["home"], f["league"])
                am = euro_metrics(euro_db, f["away"], f["league"])
                if not hm or not am:
                    continue
                for name, conf, combined in euro_matrices(hm, am):
                    found += 1
                    st.success("*" + name + " — " + str(conf) + "% confidence*  \n"
                               + f["home"] + " vs " + f["away"] + " (" + f["league"] + ")  \n"
                               + "Combined avg: " + str(round(combined, 1)) + " goals | Clusters: "
                               + euro_cluster(hm) + " vs " + euro_cluster(am))
            if found == 0:
                st.info("No matrix qualifiers right now — discipline over volume. 🤖")
        else:
            st.warning("Run euro-bot first to build euro.db.")

    allm = []
    if euro_db:
        for r in euro_q(euro_db, "SELECT DISTINCT league, home AS t FROM results UNION SELECT DISTINCT league, away FROM results"):
            m = euro_metrics(euro_db, r["t"], r["league"])
            if m:
                m["cluster"] = euro_cluster(m)
                allm.append(m)
    df_euro = pd.DataFrame(allm)

    with et2:
        st.subheader("🔥 High-Octane Offensive Cluster")
        if len(df_euro):
            st.dataframe(df_euro.sort_values("gf", ascending=False).head(15), use_container_width=True)
        else:
            st.info("No data yet. Run euro-bot workflow to start capturing MW1-2 results.")

    with et3:
        st.subheader("🛡️ Defensive Fortress Cluster")
        if len(df_euro):
            st.dataframe(df_euro.sort_values("ga").head(15), use_container_width=True)
        else:
            st.info("No data yet.")

    with et4:
        st.subheader("📊 Full Team Metrics")
        if len(df_euro):
            st.dataframe(df_euro.sort_values("diff", ascending=False), use_container_width=True)
        else:
            st.info("No data yet.")

    st.caption("Powered by Owenzo OS 🤖 | xG proxy = goals-based early-season rates")
