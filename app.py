# ============================================================
# OWENZO FOOTBALL AI — app.py (FINAL v8)
# Tabs: Predict & Slips | Tracker & ROI | Euro Hub (euro_hub.py)
# ============================================================
    ...
    if not evs:
        ...
        for e in evs:
            ...
            if not lid or not h or not a:
                continue
            (ph, pd, pa, po15, po25, pbtts), ndata = probs_for(
                _get(f"{API}/eventspastleague.php?id={lid}").get("events") or [], h, a)
            pick, conf = top_pick(ph, pd, pa, po15, po25, pbtts)
            leg, legp = top_pick(ph, pd, pa, po15, po25, pbtts, allow_draw=False)
            rows.append({"League": e.get("strLeague", ""), "Match": f"{h} vs {a}",
                         "1": int(ph * 100), "X": int(pd * 100), "2": int(pa * 100),
                         "O1.5": int(po15 * 100), "O2.5": int(po25 * 100),
                         "BTTS": int(pbtts * 100), "TOP PICK": pick,
                         "CONF": int(conf * 100), "_leg": leg, "_legp": legp,
                         "_ok": ndata >= 2})
...
with t2:
    ...
    if os.path.exists("soccer_tracker.db"):
        try:
            ...
            df = pd.read_sql("select * from slips order by rowid desc limit 60", con)
            con.close()
        except Exception:
            df = None
