# bot_ci.py v4 - Telegram bot + xG + clean version + auto TikTok video
import os
import re
import math
import json
...
def grade_market(market, selection, home, away, fh, fa):
    if market == "1X2":
        if selection == "Draw":
...
def fmt_leg(l):
    return (l.get("country", "-") + " | " + l["league"] + "\n" + l["match"] + "\n"
            + "Pick: " + l["market"] + " - " + l["selection"] + " @ " + str(l["odds"]) + "\n"
            + "Exp. score: " + l["expected_score"] + " | AI prob: "
            + str(int(l["prob"] * 100)) + "%\n")


def fmt_slip(title, slip, total, stake):
    lines = [title, "Total AI odds: " + str(total) + " | Stake: " + str(stake),
             "-" * 34, ""]
    for l in slip:
...
def _slide(items, path):
    from PIL import Image, ImageDraw, ImageFont
    img = Image.new("RGB", (1080, 1920), (11, 31, 58))
    d = ImageDraw.Draw(img)
    d.ellipse((390, 120, 690, 420), outline=(60, 190, 230), width=10)
    y = 520
    for text, size, color in items:
        try:
            f = ImageFont.truetype(FONT_BOLD, size)
        except Exception:
            f = ImageFont.load_default()
        for ln in _wrap(text, max(10, int(880 / (size * 0.62)))):
            d.text((100, y), ln, font=f, fill=color)
            y += int(size * 1.35)
        y += 50
    img.save(path)
...
def make_video(slip):
    ...
    slides = [[("OWENZO FOOTBALL AI", 72, (255, 255, 255)),
               ("AI MATCH ANALYSIS", 54, (140, 220, 140)),
               (str(datetime.today().date()), 44, (120, 190, 255)),
               ("Entertainment only | 18+", 34, (200, 200, 200))]]
    for l in slip:
        slides.append([
            (l.get("country", "") + " | " + l["league"], 42, (140, 220, 140)),
            (l["match"], 58, (255, 255, 255)),
            ("Expected score: " + l["expected_score"], 54, (140, 220, 140)),
            ("AI view: " + clean_view(l), 42, (120, 190, 255)),
        ])
    slides.append([("FULL ANALYSIS ON", 64, (255, 255, 255)),
                   ("TELEGRAM", 64, (255, 255, 255)),
                   ("t.me/owenzosoccerslips", 46, (120, 190, 255)),
                   ("Entertainment only | 18+", 34, (200, 200, 200))])
    ...
    with open(listpath, "w") as f:
        for i, s in enumerate(slides):
            ...
            _slide(s, p)
            f.write("file '%s'\nduration 2.5\n" % p)
        f.write("file '%s'\n" % os.path.join(tmp, "s%d.png" % (len(slides) - 1)))
    out = os.path.join(tmp, "tiktok.mp4")
    subprocess.run(["ffmpeg", "-y", "-f", "concat", "-safe", "0", "-i", listpath,
                    "-vf", "fps=24,format=yuv420p", out],
                   check=True, capture_output=True)
    return out


def tg_send_video(path):
    if not TG_TOKEN:
        print("[DRY RUN] video:", path)
        return
    try:
        with open(path, "rb") as f:
            requests.post("https://api.telegram.org/bot" + TG_TOKEN + "/sendVideo",
                          data={"chat_id": TG_CHAT,
                                "caption": "🎬 AI analysis - save & post 📱"},
                          files={"video": f}, timeout=90)
    except Exception as e:
        print("TG video failed:", e)


def build_slips(days):
    fixtures, _ = get_fixtures(datetime.today().date(), days)
    legs = []
    for f in fixtures:
        legs += generate_legs(model_match(f["home"], f["away"], f["league"]))
    return legs


def post_daily():
    slip, tot = build_accumulator(build_slips(1), 8, 4)
    if not slip:
        tg_send("DAILY " + str(datetime.today().date()) + ": no verified value legs today.")
        return
...
def post_vip():
    slip, tot = build_vip(build_slips(7))
    if not slip:
        tg_send("VIP " + str(datetime.today().date()) + ": not enough verified fixtures.")
        return
    stake = round(BANKROLL * 0.001, 2)
    send_long(fmt_slip("VIP WEEKLY ~200-ODDS ACCUMULATOR - " + str(datetime.today().date()), slip, tot, stake))
    log_slip("VIP", slip, tot, stake, str(datetime.today().date()))


def post_results():
    auto_grade_from_feed()
    cutoff = (datetime.now() - timedelta(days=7)).isoformat()
    rows = conn().execute(
        "SELECT slip_type, fixture_date, total_odds, stake, status, profit FROM slips WHERE settled_at >= ? ORDER BY slip_id",
        (cutoff,)).fetchall()
    lines = ["WEEKLY GRADED RESULTS - " + str(datetime.today().date()), "-" * 34]
    staked = ret = 0.0
    if not rows:
    ...
    for t, d, odds, stake, status, profit in rows:
        staked += stake
        ret += stake + profit
        lines.append(("WON " if status == "WON" else "LOST ") + t + " | " + d
                     + " | odds " + str(odds) + " | stake " + str(stake)
                     + " | P/L " + str(profit))
    profit = round(ret - staked, 2)
    lines.append("-" * 34)
    lines.append("Staked: " + str(round(staked, 2)) + " | Returned: " + str(round(ret, 2)))
    lines.append("Profit: " + str(profit) + " | ROI: "
                 + str(round(100 * profit / staked, 1) if staked else 0.0) + "%")
    g = conn().execute(
        "SELECT country AS grp, COUNT(*) AS legs, ROUND(100.0*SUM(result='WON')/COUNT(*),1) AS hr FROM legs WHERE result IN ('WON','LOST') GROUP BY country ORDER BY legs DESC LIMIT 5").fetchall()
    if g:
        lines.append("")
