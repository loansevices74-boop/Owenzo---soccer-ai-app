# bot_ci.py v7.1 - daily accumulator ALWAYS posts (value first, fallback after)
import os
import re
import sqlite3
import requests
...
def get_fixtures(days):
    ...
    for i in range(days):
        ...
        for e in evs:
            if e.get("strHomeTeam") and e.get("strAwayTeam") and \
               (e.get("strLeague") or "") not in BLOCKED_SOURCES:
...
def relaxed_legs(pred):
    ...
    add("1X2", pred["away"] + " Win", pred["p_away"], False)
...
def fmt_slip(title, slip, tot, stake):
    ...
    for l in slip:
        L += [l.get("country", "-") + " | " + l["league"], l["match"],
              ...
              + (" 💎VALUE" if l.get("value") else ""),
              "Exp. score: " + l["expected_score"] + " | AI prob: " + str(int(l["prob"] * 100)) + "%", ""]
    L += ["-" * 34, "v7 engine. Paper-trade first. Entertainment only. 18+",
...
def post_daily():
    preds = [E.model_match(f["home"], f["away"], f["league"]) for f in get_fixtures(1)]
    legs = []
    for p in preds:
        legs += E.generate_legs(p)
    ...
    if not slip:
        ...
        slip, tot = build_accumulator(legs, 8, 4)
    if not slip:
        tg_send("DAILY " + str(datetime.today().date()) + ": no fixtures today. 🤖")
        return
    stake = round(BANKROLL * 0.005, 2)
    send_long(fmt_slip("DAILY MIXED ACCUMULATOR (v7) - " + str(datetime.today().date()), slip, tot, stake))
