# ============================================================
# OWENZO FOOTBALL AI — euro_hub.py (FINAL MERGED)
# • Daily Prediction Board: top 8 + ALL tracked European leagues
# • Euro Matrix picks (O1.5 / O2.5 / DRAW) — activates at MW3
# • Live early-season clusters (API-direct, no DB dependency)
# • Silent-skip: leagues with no data never show broken boxes
# ============================================================
import time
...
def top_pick(ph, pd, pa, po15, po25, pbtts):
    c = [("Home", ph), ("Draw", pd), ("Away", pa),
         ("Over 1.5", po15), ("Over 2.5", po25), ("BTTS", pbtts)]
    ...
    for name, lid in EURO_LEAGUES.items():
        ...
        for e in league_next(lid)[:6]:
            ...
            if hg >= 1.2 and ag >= 1.2 and hc >= 0.8 and ac >= 0.8 and comb >= 2.8:
                picks.append({"Date": dd, "League": name, "Match": f"{h} vs {a}",
                              "PICK": "Over 1.5", "CONF": "85-95%"})
            if hg >= 1.5 and ag >= 1.5 and hc >= 1.0 and ac >= 1.0 and comb >= 3.2:
                picks.append({"Date": dd, "League": name, "Match": f"{h} vs {a}",
                              "PICK": "Over 2.5", "CONF": "85-90%"})
            if abs(hg - ag) <= 0.3 and hc < 1.0 and ac < 1.0 and hg <= 1.5 and ag <= 1.5:
                picks.append({"Date": dd, "League": name, "Match": f"{h} vs {a}",
                              "PICK": "Draw", "CONF": "80-89%"})
    return picks

# ---------------- RENDER ----------------
def render():
    st.subheader("🇪 European Leagues — Daily Prediction Board")
    st.caption("Top 8 + all tracked European leagues • auto-refresh 6h • 🔴 TODAY on matchdays")
    today = datetime.utcnow().strftime("%Y-%m-%d")
    shown = 0
    for name, lid in EURO_LEAGUES.items():
        fixtures = league_next(lid)
        if not fixtures:
            continue  # silent skip — board never looks broken
        res = league_past(lid)
        shown += 1
        st.markdown(f"### {name}")
        rows = []
        for e in fixtures[:6]:
            ...
            rows.append({
                "Date": ("🔴 TODAY" if dd == today else dd), "Match": f"{h} vs {a}",
                "1": f"{int(ph*100)}%", "X": f"{int(pd*100)}%", "2": f"{int(pa*100)}%",
    ...
    if mw < 3:
        st.info(f"Matrix activates at MW3 (≈ Aug 29). Currently MW{mw} — clusters building below.")
    else:
        picks = matrix_picks()
        if picks:
